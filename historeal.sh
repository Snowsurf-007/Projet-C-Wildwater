...
then
	...
	head -n 50 temp.csv > real_vol50.csv
	tail -n 10 temp.csv > real_vol10.csv
	#transforme les chiffres de la deuxiemes colones en flottant et divise
	awk '{ printf "%s %.3f\n", $1, $2 / 1000 }' temp.csv | sed 's/,/./g' > temp2.csv
	tail -n 10 temp2.csv > real_vol10.csv
	head -n 50 temp2.csv > real_vol50.csv

gnuplot <<EOF
set title "Histogramme des capacités réelles des 50 plus petites usines" font ",20" center
...
gnuplot <<EOF
set title "Histogramme des capacités réelles des 10 plus grandes usines" font ",20" center
set terminal png size 1600,1000 font "Arial,12"
set lmargin 13
set rmargin 5
set tmargin 5
...
EOF
	...
	echo "Nom usine;Capacité realimale réelle (M.m^3)" > vol_real.csv
	sort -k2 -r real_vol10.csv > temp9.csv
	sort -k2 -r real_vol50.csv >> temp9.csv
	sed  's/ /;/g' temp9.csv >> vol_real.csv
